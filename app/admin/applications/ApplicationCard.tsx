...
export default function ApplicationCard({
  ...
  return (
    <article className="bg-white border border-background-tertiary rounded-2xl p-5 space-y-4">
      {/* 헤더: 이메일 + 상태 + 메타 */}
      ...
      {app.status === 'pending' && (
        <div className="flex gap-2 pt-2 border-t border-background-tertiary">
          <button
            ...
            disabled={busy}
            className="flex-1 px-4 py-2 text-sm font-semibold text-white bg-success hover:bg-success/90 disabled:opacity-50 rounded-lg transition-colors"
          >
            {busy ? '처리 중…' : '선발'}
          </button>
          <button
