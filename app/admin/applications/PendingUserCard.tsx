...
import { buildDirectApprovalMailto } from '@/lib/copy/admin-email';
...
export default function PendingUserCard({ user }: { user: PendingUser }) {
  ...
  return (
    <article className="bg-white border border-background-tertiary rounded-2xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-background-tertiary">
        <button
          ...
          className="flex-1 px-3 py-2 text-sm font-semibold text-white bg-success hover:bg-success/90 disabled:opacity-50 rounded-lg transition-colors"
        >
          {busy ? '처리 중…' : justApproved ? '승인됨 ✓' : '직접 승인'}
        </button>
        <a
          href={buildDirectApprovalMailto(user.email)}
