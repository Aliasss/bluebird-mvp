...
0. Executive Summary
1. 문제의식 — Why
2. 이론적 근거 — Theoretical Foundation (CBT · Dual Process · Prospect Theory · CAS · Heuer · Camus)
...
6. 기술 스택 (Architecture) — Next.js 16 · Supabase · Gemini · 12 마이그레이션
...
| 카테고리 | 내용 | 검증 메커니즘 |
|---|---|---|
| 핵심 가치 (Δ 측정) | 사고 디버깅 후 *측정 가능한 통증 감소* (Δpain) | `intervention.reevaluated_pain_score - logs.pain_score` 시계열 |
| 누적 가치 (락인) | 내 사용설명서 — 사용할수록 정교해지는 자기 지도 | 30일+ 사용자에게 통증·왜곡 상관관계 surface |
...
1. **CBT** (Aaron Beck, 1976) — 인지 왜곡 5종 분류 → `lib/ai/bluebird-protocol.ts:45~84` 코드화
2. **Dual Process Theory** (Daniel Kahneman, *Thinking, Fast and Slow*, 2011) — System 1/2 (10ms vs 3s) → `BLUEBIRD_THEORY_SUMMARY.dualProcess`
3. **Prospect Theory** (Kahneman & Tversky, 1979) — 손실 가중치 λ ≈ 2.25 → `loss_aversion_signal` 0~1 추정
4. **Metacognitive / CAS Model** (Adrian Wells, 2009) — 반추·걱정 → `cas_signal.rumination`·`cas_signal.worry` 0~1
...
- **입력**: 트리거(자유 텍스트, 1자~`MAX_AI_TEXT_LENGTH`), 자동 사고(자유 텍스트), pain_score(1~5 INT)
- **DB**: `logs.trigger TEXT NOT NULL` + `logs.thought TEXT NOT NULL` + `logs.pain_score INT CHECK 1~5` (mig 04) + `logs.log_type TEXT IN ('normal','success')` (mig 12)
...
- **autonomy_score 산출** (`app/api/action/route.ts:125~127`):
  ```ts
  const noteBonus = completionNote.length > 0 ? AUTONOMY_NOTE_BONUS : 0;
  actionPayload.autonomy_score =
    calcAutonomyScore({ averageIntensity, answerCount }) + noteBonus;
  ```
  → §4.2 산식 참조.
...
- **입력**: reevaluated_pain_score (INT 1~5)
- **DB**: `intervention.reevaluated_pain_score INT CHECK 1~5` + `intervention.reevaluated_at TIMESTAMPTZ` (mig 05)
...
- **정의**: 사용자가 자동 사고 직후 매기는 *주관적 통증 강도*. CBT 임상에서 SUDS(Subjective Units of Distress Scale)의 한국어 라벨 적응.
- **범위**: **1~5 INT** (코드 정합 — mig 04 `pain_score IS NULL OR (pain_score BETWEEN 1 AND 5)`)
  - 단, 온보딩 슬라이드 Act 3-2 paragraph 2는 "통증 강도(0~10)"로 surface — 코드와 카피 간 라벨 drift. 사후 정정 SOP는 sprint 2 정의 예정 (현재 코드 source of truth = 1~5 INT).
- **DB 필드**: `logs.pain_score INT CHECK NULL OR 1~5` (mig 04)
- **수집 UI**: `/log` 페이지 — 5점 칩 selector (디자인 audit 결과 이모지 selector → 라벨+숫자 칩 *보류 항목 B1*. 인터뷰 자발 언급 트리거 시 격상 — design-realignment §5)
...
- **정의**: 사용자가 디버깅 후 24시간 시점에 다시 매긴 통증과, 디버깅 직전 통증의 차이.
- **산식**:
  ```
  Δpain = pain_score - reevaluated_pain_score
  ```
  양수 = 통증 감소 (= 효과 있음).
- **입력 데이터**:
  - `logs.pain_score` (INT 1~5, mig 04) — 사고 직후
  - `intervention.reevaluated_pain_score` (INT 1~5, mig 05) — 24h 후 (실제 6~48h 윈도우 — mig 05 코멘트)
- **측정 시점**: `intervention.is_completed = TRUE` AND `reevaluated_pain_score IS NULL` AND `review_dismissed_at IS NULL` 조건 충족 시 사용자에게 재평가 카드 노출 (mig 05 인덱스 `idx_intervention_pending_review`)
- **코드 위치**: `lib/review/delta-pain.ts` (CPO 권한 외 — 여기서는 dashboard 계산 함수 `sumPositiveDeltaPain` 사용)
- **임계·해석**:
  - 양수 누적 평균 = 사용자가 효과 보고 있음 (PMF plan §6 월별 메트릭 "평균 Δpain")
  - 0 또는 음수 = 효과 미관찰 → 분석 품질·재평가 시점 점검 트리거
- **온보딩 surface** (Act 3-2 paragraph 3): "통증이 8에서 3으로 줄었다면 5만큼 줄어든 것입니다." (출처 카피이지만 코드는 1~5 — drift 항목)

### 4.2.2 자율성 지수 (autonomy_score)

- **정의**: 사용자가 1회 디버깅 사이클을 *얼마나 적극적으로* 통과했는가 측정. 게임화 단순 카운터가 아니라 *답변 깊이 × 왜곡 강도 × 노트 작성 여부*의 합산.
- **산식** (`lib/intervention/autonomy-score.ts:8~13` + `app/api/action/route.ts:125~127`):
  ```ts
  // base 산출
  base = 10
  distortionBonus = round(averageIntensity × 5)   // 0~5
  answerBonus = min(3, answerCount)                // 0~3
  baseScore = base + distortionBonus + answerBonus  // 10~18
  ...
  // 최종
  autonomy_score = baseScore + noteBonus  // 10~33
  ```
- **입력 데이터**:
  - `averageIntensity`: 해당 log의 모든 distortion intensity 평균 (0~1)
...
- **코드 위치**: `lib/intervention/autonomy-score.ts` + `app/api/action/route.ts:125~127`
- **DB 저장**: `intervention.autonomy_score INTEGER CHECK ≥ 0` (mig 01)
- **임계·해석**: 현재 *학술 근거 없는 게임화* 산식 — PMF plan §11.3 보류 항목. 인터뷰 *해석* 답변에서 모호함·불신 ≥ 20% 시 → "AI 추정" 라벨·신뢰구간·산식 투명화 등 framing 보강 트리거.
...
- **활용**:
  - autonomy_score 입력 (averageIntensity)
...
- **구성 예시** (`bluebird_retention_mechanisms_v1.md` §2.2):
  ...
  - 섹션 3: 효과 측정 누적 ("지난 30일간 N건 디버깅, 평균 통증 감소 폭 ±X점")
  - 섹션 4: 다음 30일 가설
- **가드레일** (§2.5):
...
- **`/log`**: 자유 텍스트 trigger + thought + pain_score (1~5 INT) 입력 → `logs` INSERT → `/analyze/[id]` redirect.
...
- **기능**: reevaluated_pain_score 입력 (1~5).
...
- **Supabase PostgreSQL** — 12 마이그레이션 적용.
...
| 통증 vs 스트레스 라벨 | 인터뷰 자발 언급 ≥ 60% 우세 | 라벨 변경 |
...
| product-designer | 디자인 audit·디자인 시스템 토큰 | CPO |
...
supabase/migrations/04_logs_pain_score.sql      — pain_score INT 1~5
supabase/migrations/05_intervention_reevaluation.sql — Δpain 컬럼 3종
...
| Δpain 산식 | mig 04·05 + 온보딩 Act 3-2 | ✓ (단 라벨 drift 1~5 vs 0~10) |
| autonomy_score 산식 | `lib/intervention/autonomy-score.ts:8~13` + `app/api/action/route.ts:125~127` | ✓ |
...
| 6 학술 백본 | research README + bluebird-protocol.ts + 온보딩 caption | ✓ |
| 12 마이그레이션 | `supabase/migrations/01~12` | ✓ |
...
1. **pain_score 라벨 drift**: 코드(mig 04 + analyze API)는 INT 1~5, 온보딩 슬라이드 Act 3-2는 "0~10" surface. 인터뷰 자발 언급 측정 후 라벨 정정 결정 (PMF plan §11.3 통증 라벨 트리거).
2. **본 문서가 사용한 사례 수치**: 온보딩 캡션 "통증이 8에서 3으로 줄었다면" 카피는 1~5 코드와 정합 X — 카피 수정 BackLog (sprint 2).
3. **autonomy_score 산식 학술 근거 부재**: PMF plan §11.3 trigger — 인터뷰 모호함·불신 ≥20% 시 framing 보강 작업.
