...
| 다크모드 | 인터뷰 자발 언급 ≥10% |
| 별도 콘텐츠 피드 공간 | 인터뷰 학습 자원 자발 요청 ≥10% (coding-rubric C10) → 대안 A3(`/our-philosophy` refresh) 또는 B(분석 결과 페이지 학습 카드) 검토 격상 (출처: `docs/meetings/2026-05-04-content-feed-review.md`) |
...
- **위험**:
  ...
  - **(2026-05-04 추가, 해외 사례 분석 v1)** *해외 동일 메커니즘 케이스 단명·축소 패턴*:
    - **Quirk** (US, YC S19, 인지 왜곡 자가 태깅 = BlueBird 직접 조상) — B2C monetization 실패 → FreeCBT (GPL 비영리) 전환
    - **Woebot** (US, CBT 챗봇) — 2025-06-30 B2C 종료, 사유: FDA 인가 비용 + LLM 등장. 엔터프라이즈로 전환
    - 패턴: *단일 메커니즘만으로는 B2C lockin 어렵다* + *FDA·식약처 압력은 "법적 안전"이 아니라 "생존" 결정*
- **완화**:
  ...
  - **(2026-05-04 추가)** *차별화 3축 결합 작동 가드* — 분석가 톤 + 신체화 매핑 + 자동화 저가는 *각각이 아니라 결합*으로만 lockin (Quirk·Woebot 단일 메커니즘 실패 패턴 회피)
  - **(2026-05-04 추가)** *이탈 의도 시그널 측정* — `coding-rubric-v1-2026-05-04.md` C11 신규 (≥30% "나아지면 그만 쓸 것" → 락인 가설 위기 경보) — 무한 자가관리 모델(Quirk·FearTools) 중력 회피
(출처: `bluebird_competitive_strategy_v1.md` §7 + §8 (2026-05-04 신규))
