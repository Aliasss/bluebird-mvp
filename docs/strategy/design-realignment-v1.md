...
- [`bluebird_retention_mechanisms_v1.md`](./bluebird_retention_mechanisms_v1.md) — 매뉴얼·통계 맥락화 (베타 데이터 스키마만)
...
| CTO 코드 리뷰 + GO 판정 (1차) | 완료 | 2026-04-29 | — |
| 1차 타겟 정의 정정 — 직무 enumerate 제거 + 3축 분리 | 완료 | 2026-04-30 | `64ad52f` |
| product-designer 모바일 UX 검토 + CPO 재검토 | 완료 | 2026-04-30 | — |
| P0 모바일 후속 — ✦ 글리프·시나리오 어휘·viewport·footer·lint 가드 | 완료 | 2026-04-30 | `0035943` + `b481dcf` |
| CTO P0 후속 — `<ul role="list">` + footer touch target 32px + 의료기기 식별자 점검 CLEAN | **완료** | 2026-04-30 | (본 commit) |
| 시니어 PO 정식 합류 (CPO 산하 — designer + PO 2명 구성) | 완료 | 2026-04-30 | (본 commit) |
| 리텐션 메커니즘 v1 — `bluebird_retention_mechanisms_v1.md` strategy 디렉토리 통합 | 완료 | 2026-04-30 | (본 commit) |
| 4 임원 전원 합의 (a)+(c) — 데이터 스키마 + 통계 수집(미노출) | 완료 | 2026-04-30 | (본 commit) |
| senior-fullstack DB schema migration — user_patterns + user_aggregates_daily + 익명 hash + RLS + 표본 가드 | 진행 중 → 완료 | 2026-04-30 | (본 commit) |
| M30.1 인터뷰 가이드 §3 자발 언급 항목 2 추가 (매뉴얼·통계 기대치) | 완료 | 2026-04-30 | (본 commit) |
| 자발 언급 ≥10% 트리거 — 매뉴얼·통계 prototype 격상 메커니즘 명시 | 완료 | 2026-04-30 | (본 commit) |
| **B3 인터뷰 split 마커 — Deploy 시점** | 2026-04-30 | `b481dcf` | sample funnel 카피 변경 후 deploy. 인터뷰 분석 시 split 기준 |
| iPhone SE 320px / iOS PWA standalone 실기 테스트 | 대기 | IM.1 베타 모집 전 G1 | — |
