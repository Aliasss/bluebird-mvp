...
**작성일:** 2026-05-09 (Section 0 추가: 2026-05-10)
**작성자:** Operator (CPO·CTO 양 관점 통합)
**스펙 상태:** Brainstorming 완료, 구현 plan 작성 직전, IM.1 D-3 G1 production 배포 완료
...
> **이 문서를 처음 여는 분께:** Section 0은 일상어 narrative입니다. 시스템이 뭘 하는지 5분 안에 이해하려면 §0만 읽으세요. Section 1~6은 Product·Tech 양 관점의 상세 의사결정·구현 레퍼런스입니다.

---

## 0. 직관 이해 — 이 시스템이 뭘 하는가

### 0.1 한 줄 요약

> **"오늘 저녁 체크인 빼먹은 사용자에게, 매일 밤 9시에, 단 한 번 말을 거는 시스템."**

세 가지 결정으로 분해되는 한 줄:
- **누구에게:** 푸시 켠 사용자 중 **오늘 evening 체크인이 없는 사람만**
- **언제:** 매일 **21:00 KST** (UTC 12:00, 한국 DST 없음)
- **얼마나:** **하루 1회**, 재발송 없음
...
### 0.2 사용자 시나리오 3개 — 실제로 무슨 일이 벌어지나

#### 시나리오 ① 민지 — 가장 자연스러운 경로

```
[Day 1 저녁]
가입 → 온보딩 → 첫 이브닝 체크인 완료 → 대시보드로 자동 이동
   ↓
대시보드 상단에 카드:
   ┌────────────────────────────────────────┐
   ...
   └────────────────────────────────────────┘
   ↓
[지금 켜기] → 브라우저 native 다이얼로그 → "허용"
   ↓
토스트: "켰습니다. 21시에 안 했으면 알림이 와요."
...
│ 1분이면 한 줄 남길 수 있어요...        │
────────────────────────────────────
탭 → BlueBird PWA 열림 → 곧장 /checkin → 1분 만에 한 줄 남기고 streak 유지
```

#### 시나리오 ② 준호 — 미루다 나중에 켠 케이스

```
[Day 1] 가입 → 첫 체크인 → 카드 노출 → "나중에" 클릭
        → 카드 사라짐, 영구 미노출 (반복 노출은 피로도만 늘림)

[Day 4] 대시보드 진입
        Streak 카드 아래 한 줄 띠 ─────────────────
        │ 21시 미체크인 알림 켜기 — Streak 보호 [켜기] ✕ │
        ──────────────────────────────────────────
        → [켜기] → 다이얼로그 → "허용" → 구독 완료
        (✕로 닫으면 7일간 침묵, 그래도 안 켜면 또 노출)
```

#### 시나리오 ③ 선영 — 거부한 케이스

```
[Day 1] 첫 체크인 → 카드 → "지금 켜기" → 다이얼로그 "허용 안 함"
   ↓
   토스트: "알림이 차단됐어요. 브라우저 설정에서 풀어야 다시 켤 수 있어요."

   영구 미수신. 우리는 더 이상 권유하지 않음.
   (반복 권유 = 짜증 = 이탈 위험. 거부한 사용자의 의사 존중.)
```

### 0.3 권한 요청을 거는 두 위치 — "P2 / P3"의 자연어 매핑

본 문서 본문의 **P2·P3** jargon은 다음 두 위치를 가리킵니다:

| 별칭 | 자연어 명칭 | 위치 | 노출 시점 | 역할 |
|---|---|---|---|---|
| **P2** | "체크인 직후 카드" | 대시보드 상단 (체크인 직후 redirect됐을 때) | **첫 체크인 1회 한정** | 강한 첫인상 — 가치를 방금 경험한 직후 권유 |
| **P3** | "대시보드 배너" | 대시보드 Streak 카드 아래 | 카드 못 봤거나 "나중에" 누른 사용자 회복용. 7일 silence | 안전망 — 첫 카드 놓친 사용자가 영구 미수신되지 않게 |

**왜 두 개를 분리했나?**
- 카드 하나만 두면 → 미루는 사용자 모두 영구 미수신 → 표본 손실
- 배너 하나만 두면 → 가치-요청 결합이 약함 → 동의율 낮음
- → **카드 = 강한 첫인상 + 배너 = 안전망** 역할 분담
...
| 안 | 채택 | 이유 |
|---|---|---|
| 모두에게 매일 21시 발송 | ❌ | 이미 한 사람한테도 가면 = 짜증 → 차단 → 표본 손실 |
| **이브닝 미체크인만** | ✅ | "필요한 사람에게만" 원칙. 베타 30명 표본 보호의 핵심 |
| 사용자별 시각 설정 | ❌ Phase 2 | 1인 운영 MVP에 과도. IM.1 데이터로 검증 후 격상 |
| 학습형(스마트) | ❌ Phase 2 | 데이터 0 상태에서 학습 불가 |

#### ② 왜 21시인가

- 한국인 저녁 활동 마감 직전 = 하루 회고 가능한 시간대
- 자기 전이라 행동 trigger 잘 됨
- 한국 DST 없음 → 시간 변경 이슈 0
- 단일 시각이라 운영(cron) 슬롯 1개로 충분
...
| 의도 | 분석 |
|---|---|
| "안 됐어요" — 사실 진술 | BlueBird 정체성: 응원·위로 톤 ❌ / 분석·관찰 톤 ✅ |
| "1분이면" — 비용 명시 | 사용자가 "들이는 시간이 얼마지?" 의문 즉시 해소 |
| "지난 7일과 비교 데이터" — 가치 구체화 | 추상 응원("힘내세요!") ❌ / 도구적 가치("데이터 쌓임") ✅ |
| ✦✧★☆ 이모지·"오늘 하루도"·"함께" 어휘 | `lint-copy`로 코드에 못 들어가게 차단됨 (§2.4) |

#### ④ 왜 이메일·SMS는 안 만드나

| 채널 | 채택 | 이유 |
|---|---|---|
| Web Push | ✅ | 무료, 도메인만 있으면 작동, PWA와 자연 결합 |
| 이메일 | ❌ | 마케팅 동의 별도 수집 필요, 도달률·인박스 분류 불확실, SMTP 인프라 추가 부담 |
| SMS | ❌ | 1건당 비용, 1인 운영자에게 과도 |
| 인앱 | ❌ | 이미 대시보드 노출로 충족 |

원칙: **"Web Push만으로 안 되는 알림은 만들지 않는다"** (§5.4 결정잠금).

#### ⑤ 왜 iOS 일부 사용자 도달 손실을 받아들였나

iOS는 **홈 화면에 추가한 PWA 안에서만** 푸시 받음. 일반 Safari 안 됨. Apple 정책 → 우리가 못 바꿈.

| 옵션 | 채택 | 이유 |
|---|---|---|
| 도달 손실 수용 + 베타 모집 시 PWA 설치 안내 | ✅ | BlueBird 페르소나 = "분석가형". PWA 설치 정도는 받아들일 가능성 높다는 가설 |
| Native iOS 앱 개발 | ❌ | App Store 심사·유지보수. MVP 범위 외 |
| iOS 사용자 베타 제외 | ❌ | 표본 손실 더 큼 |

### 0.5 측정 지표 — 일상어 번역

| 시스템 표현 | 일상어 | 합격선 (가설) |
|---|---|---|
| `p2_unique_shown` | "처음 카드 본 사람 수" | (분모) |
| `unique_granted / p2_unique_shown` | "카드 본 사람 중 켠 비율" (= **권한 동의율**) | **60% 이상** |
| `unique_denied / p2_unique_shown` | "카드 본 사람 중 거부한 비율" (= **권한 거부율**) | **25% 이하** (이상이면 카피 결함 신호) |
| `unique_subscribed` | "서버에 정상 등록된 사람 수" | granted와 일치해야 정상 |
| `total_sent` | "21시 발송 총 횟수 (사용자 × 일수)" | 단순 모니터링 |
| `total_clicked / total_sent` | "알림 받고 실제 들어온 비율" (= **알림→체크인 전환율**) | **25% 이상** |

집계 view: `v_push_funnel` (service_role 전용). IM.1 D7·D14에 1회씩 SQL 한 줄로 산출.

### 0.6 안 만든 것 (Phase 2 backlog) — IM.1 종료 후 데이터 보고 결정

1. **휴면 사용자(7일 미사용) 자동 게이팅** ← 1순위. 안 쓰는 사람 그만 깨우기
2. **재평가 리마인더** (6~48h 후 통증 재측정 유도) — 본 인프라 그대로 재사용
3. **사용자별 시각 설정** — IM.1에서 21시 가설 검증되면
4. **A/B 카피 실험** — `notification_events.metadata.variant` 1줄 추가만 하면 가능
5. **위기 follow-up** (24h 안전 확인) — 법적 검토 별도

전체 8개 항목 §5.3 참조.

### 0.7 한 줄 정리 다시

> **민지가 깜빡한 날 21시에, 본인 잠금화면에서, "오늘 체크인 안 됐어요"라는 분석 톤 알림 한 번 받고, 탭 한 번에 1분 안에 기록 마치고, streak 유지하는 시스템.**
>
> 켜는 시점은 본인이 가장 가치를 느낀 직후 (첫 체크인 후 카드) + 놓친 사람 회복용 (대시보드 배너).

이걸 베타 30명 × 14일 동안 돌려서, **"처음 본 사람 중 60% 이상이 켜고, 거부율은 25% 이하, 알림 받은 사람 중 25% 이상이 실제로 체크인하는가"**를 검증하는 것이 IM.1 푸시 인프라의 미션.
