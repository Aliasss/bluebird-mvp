...
### 3.4 _actions.md 갱신 (SHALL — 강제)

§3.1에서 추출한 모든 actions를 `_actions.md` Open 섹션에 1:1 추가. 누락 0건 강제.
§3.2에서 close 판정한 항목은 Closed (this week) 섹션으로 이동.

### 3.5 Pre-commit Verify 가드 (BLOCK if mismatch)

§Phase 5 진입 *前* 다음 검증 강제:

```bash
# 회의록 §4 Action items 섹션의 새 entry count
ACTIONS_IN_MEETING=$(awk '/^## 4\. Action items/,/^## 5\./' "$FILE" | grep -cE '^- \[ \]')

# _actions.md Open 섹션의 이번 routine 출처 신규 추가 count
ACTIONS_IN_TRACKER=$(grep -cE "출처: ${TODAY} standup" docs/meetings/_actions.md)

if [ "$ACTIONS_IN_MEETING" -ne "$ACTIONS_IN_TRACKER" ]; then
  echo "[$(TZ=Asia/Seoul date '+%Y-%m-%d %H:%M:%S KST')] BLOCKED — Phase 3 미완: 회의록 §4 ${ACTIONS_IN_MEETING}개 vs _actions.md ${ACTIONS_IN_TRACKER}개 불일치" >> docs/meetings/_heartbeat.log
  exit 1
fi
```

mismatch 발견 시 routine 즉시 중단, commit·push 안 함. 작업물 working tree 보존.
...
회의록을 *작성하기 전*에 path를 확정. Phase 5.1·5.5 모두 동일 `$FILE` 변수 사용.
...
# 이 시점부터 $FILE 은 collision-safe path. Phase 5.1의 회의록 작성, 5.5의 git add 모두 이 변수 사용.
```

### 5.1 회의록 작성

저장 위치: `$FILE` (Phase 5.0에서 결정. 기본값 `docs/meetings/{YYYY-MM-DD}-standup.md` KST 기준 일자, 충돌 시 `-1.md`, `-2.md` 순차 suffix)
