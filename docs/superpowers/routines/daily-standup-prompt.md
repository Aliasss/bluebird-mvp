...
| 조건 | 모드 | Phase 2 동작 |
|---|---|---|
| agenda 0 + git 24h 변화 0 + 최근 3일 standup에 `## 0. Quiet day` 헤더 0~2회 | **Case 1 — 완전 조용** | 임원 4명 standing items만, 산하 0명 |
| agenda 0 + git 24h 변화 0 + 최근 3일 standup에 `## 0. Quiet day` 3회 (연속) | **Case 3 — 정체 감지** | CPO에 emergent agenda 자동 추가("3일째 git 0 + agenda 0. 무엇이 막고 있는가?") 후 Case 1 진행 |
| agenda 0 + git 일부 변화 | **Case 2 — 부분 조용** | 정상 — 매핑된 산하 + 임원 4명 |
| agenda 있음 또는 git 변화 있음 | **정상** | 매핑된 산하 + 임원 4명 + agenda 호명자 |
