...
function buildAnalysisPrompt(input: {
  ...
  return [
    ...
    '- trigger_category는 trigger 문장의 도메인을 12개 중 하나로 라벨링한다.',
    '',
    '  [기본 8개 — 도메인 분류]',
    '  · work: 직장 업무, 상사·동료, 회사, 커리어, 면접, 출근, 발표, 마감',
    '  · relationship: 친구, 연인, 지인, 데이트, 이별, 비연애 인간관계 갈등',
    '  · family: 부모, 형제자매, 자녀, 배우자, 친척',
    '  · health: 신체 컨디션(일반), 통증, 질병, 의료 검진, 식이 (수면 단독은 sleep_rumination 우선)',
    '  · self: 자존감, 정체성, 외모, 성격, 능력 자기평가',
    '  · finance: 돈, 소비, 부채, 투자, 노후, 가격 결정',
    '  · study: 학교, 시험, 과제, 자격증, 진학',
    '  · other: 12개 모두에 해당하지 않을 때만 사용',
    '',
    '  [확장 4개 — 인지 패턴 특화. 2026-05-16 추가.',
    '   다음 4개는 *내용 도메인*이 아니라 *작동하는 인지 패턴*으로 분류한다.',
    '   기본 8개와 겹치면 확장 4개를 우선 (예: "발표 망칠까 잠 안 옴" → sleep_rumination)]',
    '  · sleep_rumination: 잠 안 옴 + 반복 사고. "잠자려는데", "자정", "새벽", "또 못 자면", "내일 일찍 일어나야" 같은 시간 단서 동반. Wells (2009) CAS 정합.',
    '  · decision_paralysis: 선택 못 함 + 분석 마비. "결정 못 하겠어", "이걸 할지 저걸 할지", "선택지가", "고민만 하다", "어쩌지", "도저히 못 정하겠다" 같은 결정 회피 어휘.',
    '  · social_comparison: SNS/동료/주변과 자기 비교. "친구는", "남들은", "인스타", "또래에 비해", "동기는", "남들 다 ~한다" 같은 외부 비교 단서.',
    '  · avoidance_accumulation: 같은 상황 반복 회피 누적. "또 피했어", "이번에도 못 했다", "매번 미루는 것 같다", "벌써 N번째 안 갔다" 같은 반복+회피 단서. Struijs (2017) 9년 종단 정합.',
    '',
    '- 모호하면 가장 dominant한 인지 패턴(확장 4) 또는 도메인(기본 8) 1개를 고른다. 절대 복수 라벨 금지.',
    '- 반드시 12개 중 하나만 사용. 새 라벨 생성 금지.',
    '',
    '[Few-shot Examples]',
